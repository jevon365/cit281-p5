...
const Monster = require("./p5-monster.js").Monster;
...
module.exports = class MonsterGame {
  ...
  */
  ...
  // asynic anables the us of key terms to be used in the promis, 
  //the promis is saying that this code will execute even if there is an error
  async playGame(GameDelay) {
    ...
    do {
      ...
      this.monster.forEach(monster =>{
        console.log(`monster:${monster.monsterName},`+` min Life:${monster.minimumLife},`
        +`current life:${monster.currentLife},`+`satus:${monster.isAlive ? 'Alive':'Dead'}`)
      });
...
// this promis is linked to the asynic and the await
function sleep(ms) {
